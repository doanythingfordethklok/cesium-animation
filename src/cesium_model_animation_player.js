...
export class AnimationParser {
  ...
  static async parseAnimationSetFromUri (glb_uri) {
    const array_buffer = await this._getResourceAsync(glb_uri);

    return this._parseAnimationSetFromArrayBuffer(array_buffer);
  }

  static async parseAnimationSetFromFile (glb_file) {
    const array_buffer = await this._readFileAsync(glb_file);

    return this._parseAnimationSetFromArrayBuffer(array_buffer);
  }

  static _parseAnimationSetFromArrayBuffer (array_buffer) {
    const animation_nodes = AnimationParser.parseAnimationNodesFromArrayBuffer(array_buffer);
    ...
    const animations = AnimationParser.parseAnimationsFromArrayBuffer(array_buffer);
